...
export default function PullPage() {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        // Filter and transform the config for pull exercises
        const pullGroups = data.muscleGroups
          .filter((group: MuscleGroup) => group.category === 'pull')
          .reduce((acc: { [key: string]: string[] }, group: MuscleGroup) => {
            acc[group.name] = group.exercises.map(ex => ex.name);
            return acc;
          }, {});
        
        setMuscleGroups(pullGroups);
      } catch (error) {
 ...
 const fetchAllDates = async () => {
    try {
      const res = await fetch('/api/get-dates?type=pull');
      const data = await res.json();
